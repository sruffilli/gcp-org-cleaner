...
def delete(organization_id, exclude_custom_roles, dry_run):
  ...
  """
  logger.info(f"Starting processing custom roles")

  custom_role_list = _list_custom_roles(organization_id)

  logger.info(f"Retrieved {len(custom_role_list)} custom role(s)")

  exclude_custom_roles_list = exclude_custom_roles.split(",") if exclude_custom_roles else []
  ...
  for role in custom_role_list:
    ...
    log_message = "%sDeleting custom role %s ." % ("(Simulated) " if dry_run else "", role.name)
    logger.info(log_message)

    if not dry_run:
      _delete_custom_role(organization_id, role_id)

  logger.info(f"Done processing custom roles")
